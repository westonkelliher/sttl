...
merges the two passes into clean text segment by segment while you talk
(the merged transcript trails live speech by ~one chunk). On stop, the raw
transcript hits the clipboard in ~1s; the refined one replaces it seconds later.

Opening the UI starts recording immediately — `sttl -n` (or `?noauto=1`) opens
idle. Domain terms in `~/.config/sttl/keywords.md` (STTL_KEYWORDS) are fed to
the merge LLM to bias ambiguous words toward your vocabulary.
